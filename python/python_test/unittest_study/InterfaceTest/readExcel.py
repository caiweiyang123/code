...
class ReadExcel():
    def get_xls(self, xls_name, sheet_name):  # xls_name填写用例的Excel名称 sheet_name该Excel的sheet名称
        ...
        file = open_workbook(xlspath,encoding_override='utf-8')  # 打开用例Excel

        sheet = file.sheet_by_name(sheet_name)  # 获得打开Excel的sheet
        # 获取这个sheet内容行数
        nrows = sheet.nrows
        for item in range(nrows):  # 根据行数做循环
            # 如果这个Excel的这个sheet的第i行的第一列不等于case_name那么我们把这行的数据添加到cls[]
            if sheet.row_values(item)[0] != u'case_name':
                cls.append(sheet.row_values(item))
        return cls


if __name__ == '__main__':  # 我们执行该文件测试一下是否可以正确获取Excel中的值
    print(ReadExcel().get_xls("userCase.xls", "login"))
    print(ReadExcel().get_xls('userCase.xls', 'login')[0][1])
    print(ReadExcel().get_xls('userCase.xls', 'login')[1][2])